...
class FreeplaySelectState extends MusicBeatState {
	...
	{
		#if desktop
		DiscordClient.changePresence("In the Continued Freeplay Menu", null);
		#end
		...
		CurrentSongIcon = new FlxSprite().loadGraphic(Paths.image('week_icons_' + (AllPossibleSongs[CurrentPack].toLowerCase())));

		CurrentSongIcon.centerOffsets(true);
	...
	public function LoadProperPack()
		{
			switch (AllPossibleSongs[CurrentPack].toLowerCase())
			{
				...
				case 'extra':
					...
					addWeek(['Coolswaggy'], 8, ['coolguy']);
					addWeek(['Having Fun'], 8, ['fun']);
					addWeek(['Coolness'], 8, ['coolguy']);
					addWeek(['Funni'], 8, ['funni']);
					addWeek(['Incident'], 8, ['ohno']);
					addWeek(['-'], 8, ['minus']);
					addWeek(['Opportune'], 8, ['oppo']);
					addWeek(['Deleted'], 8, ['deletedbozo']);
					addWeek(['Buddies'], 8, ['fun']);
					addWeek(['Unbreakable'], 8, ['unbreak']);
					addWeek(['Doomed'], 8, ['doom']);
					addWeek(['Extinguish'], 8, ['extin']);
					addWeek(['Unscrewed'], 8, ['happibambi']);
					addWeek(['Unrestrained'], 8, ['unrest']);
					addWeek(['Crypt'], 8, ['crypto']);
				case 'joke':
					addWeek(['Deez'], 8, ['deez']);
					addWeek(['Battle Pass'], 8, ['fortnite']);
					addWeek(['Goofy Ahhd'], 8, ['goofy']);
					addWeek(['Stoopeed'], 8, ['stoped']);
					addWeek(['Hoydjz'], 8, ['hoydjz']);
			}
		}
	...
	override function beatHit()
	{
		...
		for (i in 0...iconArray.length)
		{
			...
			iconTween = FlxTween.tween(iconArray[i].scale, {x: 1, y: 1}, Conductor.crochet / 1300, {ease: FlxEase.quadOut});
		}
